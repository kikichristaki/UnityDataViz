...
public class heatmapMaker1 : MonoBehaviour {
	...
	private void createPoints(){
		...
		for (int x = 0 ; x < resolution ; x++){
			for (int z = 0 ; z < resolution; z++){
				...
				points [i].position = p;
				points [i].startColor = new Color (p.y, 0f, p.z);
				points [i++].size = 0.07f;
			}
		}
...
//	}
	...
	private static Vector3[] positionstable(){
		Vector3[] positions = new Vector3[10000];
		for (int i = 0; i < positions.Length; i++){
			positions[i] = new Vector4(Random.Range(-0.4f, +0.4f), Random.Range(-1f, +1f), Random.Range(-0.4f, +0.4f));
